...
const SELECTORS = Object.freeze({ root: "[data-split-scroll]" });
const EVENTS = Object.freeze({ scroll: `${COMPONENT_NAME}:scroll` });
...
function normalizeOptions(options) {
	...
	return {
		...
		rootMargin: typeof options.rootMargin === "string" && options.rootMargin.trim() ? options.rootMargin : DEFAULT_OPTIONS.rootMargin,
		threshold: Number.isFinite(threshold) && threshold >= 0 && threshold <= 1 ? threshold : DEFAULT_OPTIONS.threshold,
...
function supportsViewTimeline(view = window) {
	return Boolean(typeof view.ViewTimeline === "function" && view.CSS?.supports?.("animation-timeline: view()") && view.CSS.supports("animation-range: cover 0% cover 100%"));
}
function setFallbackState(entry, threshold, once) {
	...
	element.dispatchEvent(new CustomEvent(EVENTS.scroll, {
		bubbles: true,
		detail: {
			visible,
...
function scrollRevealAll(selector = SELECTORS.root, options = {}) {
	if (typeof document === "undefined") return () => void 0;
	const normalized = normalizeOptions(options);
	const elements = Array.from(document.querySelectorAll(selector));
	if (!elements.length) return () => void 0;
	const view = document.defaultView;
	const hasOnceElements = normalized.once || elements.some((element) => element.hasAttribute("data-scroll-once"));
	...
	const IntersectionObserverConstructor = view?.IntersectionObserver ?? globalThis.IntersectionObserver;
	...
	const observer = new IntersectionObserverConstructor((entries) => {
		entries.forEach((entry) => {
			if (setFallbackState(entry, normalized.threshold, normalized.once)) observer.unobserve(entry.target);
		});
	}, {
...
//#endregion
export { scrollRevealAll, supportsViewTimeline };

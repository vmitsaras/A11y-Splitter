import { t as A11yTextSplit } from "./src-D22P1dQP.js";
...
const fontReadyRefreshed = /* @__PURE__ */ new WeakSet();
function groupByRenderedLine(elements, tolerance) {
	const lines = [];
...
const installLines = (instance) => {
	const visual = instance.visualElement;
	if (!visual) return null;
	const document = visual.ownerDocument;
	const view = document.defaultView;
	const lines = groupByRenderedLine(Array.from(visual.children), instance.options.lineTolerance);
	...
	const observer = typeof ResizeObserverConstructor === "function" ? new ResizeObserverConstructor(refreshWhenWidthChanges) : null;
...
//#endregion
export { groupByRenderedLine, installLines };
